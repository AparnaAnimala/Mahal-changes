    ...
    const fetchMaster = async (category) => {
      try {
        const res = await fetch(
          `http://192.168.2.22:5000/api/profile/master/${category}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`
...
const submitChangeRequest = async (section, newData, extra = {}) => {
  ...
  const url =
    roleLower === "restaurant"
      ? "http://192.168.2.22:5000/api/profile/request-change-restaurant"
      : "http://192.168.2.22:5000/api/profile/request-change-supplier";
  ...
  const loadBranchDropdown = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://192.168.2.22:5000/api/profile/${roleLower}/branch/list/${id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`
  ...
  const loadRestaurantBranchDropdown = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://192.168.2.22:5000/api/profile/restaurant/branch/list/${id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      if (roleLower === "supplier") {
        ...
        const r = await fetch("http://192.168.2.22:5000/api/profile/supplier/store", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`
      ...
      if (roleLower === "restaurant") {
        ...
        const r = await fetch("http://192.168.2.22:5000/api/profile/restaurant/store", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`
