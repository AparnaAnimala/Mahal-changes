...
const handleSearch = text => {
  ...
  fetch("http://192.168.2.21:5000/api/search/log", {
    method: "POST",
  ...
  const loadDefaultLocation = async () => {
    try {
      const res = await fetch(
        `http://192.168.2.21:5000/api/location/address/default/${restaurantId}`
      );
  ...
  const detectLocation = () => {
    navigator.geolocation.getCurrentPosition(
      async pos => {
        ...
        await fetch("http://192.168.2.21:5000/api/location/address", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
